...
void Selecao(criatura &player, condicionais &condicao, int &chave) {
    ...
    case 1:
		chave = 1;
        CriarCriatura(player, nome, "cavaleiro", 60, 7, 16, sorte, 0, "none", "none", "none", "none"); //respectivamente variavel struct, nome, tipo, vida, ataque, defesa, sorte, furtividade e 4 tags para combate
        pegar_arma(player, "Espada longa", 7);
        pegar_armadura(player, "Armadura desgastada", 16, 0);
...
void MenuPrincipal() {
	...
	do {
		...
		case 1:
			system("cls");
			Selecao(player, condicao, chave);
			break;
		case 2:
			system("cls");
            CarregarJogo(player, condicao, chave);
            ChaveDeAcesso(player, condicao, chave);
			break;
		case 3:
			system("cls");
